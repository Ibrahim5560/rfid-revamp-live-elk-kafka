...
@WithMockUser
@EmbeddedKafka
class RfidRevampLiveElkKafkaKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/rfid-revamp-live-elk-kafka-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/rfid-revamp-live-elk-kafka-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/rfid-revamp-live-elk-kafka-kafka/unregister"));
                return;
            }
        }
